...
execute unless score #temp_secret dp_vmn.temp matches 3162 as @e[tag=dp_vmn_ACTIVE_1,type=minecraft:snowball] if score @s dp_vmn.random = @e[type=minecraft:marker,tag=dp_vmn_TEMP,tag=dp_vmn_MAIN,distance=..0.001,limit=1,sort=nearest] dp_vmn.random on origin run tag @s add dp_vmn_TEMP

...
execute if score #temp_secret dp_vmn.temp matches 3161 run tellraw @a [{"text":"Chained Harvesting: ","color":"gold"},{"text":"Admin must /reload datapack ","color":"red"},{"text":"→[MISSING CONST ENTITY]","color":"gray","underlined":true,"hoverEvent":{"action":"show_text","contents":[{"text":"Potential Causes\n","color":"gold"},{"text":"CONST persistent entity was removed via /kill or other means (includes 1 armor stand entity located outside build limit in world spawn chunk. Doesn't affect non-datapack functions)","color":"gray"}]}}]

...
execute if score #temp_secret dp_vmn.temp matches 3156 unless score #const_count dp_vmn.temp matches -1 positioned ~ ~10001 ~ as @e[type=minecraft:armor_stand,tag=dp_vmn_CONST] unless data entity @s equipment.mainhand.id run item replace entity @s weapon.mainhand with minecraft:stone_button
